...
// Confirmation phrase obligatoire pour empêcher toute suppression accidentelle
...
serve(async (req) => {
  ...
  try {
    ...
    // Rate limit anti-brute-force : 5 tentatives / heure
    const rl = await checkRateLimit(user.id, 'delete-account', 5, 3600);
    if (!rl.allowed) return rateLimitResponse('delete-account', rl.retryAfter, corsHeaders);
    ...
    if (!parsed.success) {
      return new Response(
        JSON.stringify({ error: "Confirmation requise : envoyer { confirmation: 'SUPPRIMER MON COMPTE' }" }),
