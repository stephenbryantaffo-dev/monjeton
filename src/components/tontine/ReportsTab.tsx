...
const ReportsTab = ({ tontines }: Props) => {
  ...
  const exportPDF = () => {
    ...
    const cyclePages = targetCycles.map((cycle) => {
      ...
      return `<div style="margin-bottom:40px;page-break-inside:avoid"><h2 style="color:#7CFF3A;margin-bottom:4px">Cycle ${cycle.cycle_number} — ${esc(cycle.period_label)}</h2><p style="color:#888;font-size:13px">Du ${new Date(cycle.start_date).toLocaleDateString("fr-FR")} au ${new Date(cycle.end_date).toLocaleDateString("fr-FR")}</p><div style="display:flex;gap:20px;margin:16px 0"><div style="background:#14171C;border-radius:12px;padding:12px 20px;flex:1;text-align:center"><p style="color:#888;font-size:12px">Total attendu</p><p style="font-size:20px;font-weight:bold">${fmt(cycle.total_expected)}</p></div><div style="background:#14171C;border-radius:12px;padding:12px 20px;flex:1;text-align:center"><p style="color:#888;font-size:12px">Total collecté</p><p style="font-size:20px;font-weight:bold;color:#7CFF3A">${fmt(cycle.total_collected)}</p></div><div style="background:#14171C;border-radius:12px;padding:12px 20px;flex:1;text-align:center"><p style="color:#888;font-size:12px">Taux</p><p style="font-size:20px;font-weight:bold;color:${pctCollected >= 80 ? "#7CFF3A" : "#f59e0b"}">${pctCollected}%</p></div></div><table style="width:100%;border-collapse:collapse">${rows}</table></div>`;
    }).join("");
