...
const menuItems = [
  { icon: User, label: "Mon profil", path: "/settings" },
...
const Settings = () => (
  <DashboardLayout title="Paramètres">
    ...
    <div className="glass-card rounded-2xl p-5 mb-4 flex items-center gap-4">
      ...
      <div>
        <p className="text-lg font-bold text-foreground">Utilisateur</p>
        <p className="text-sm text-muted-foreground">Plan Pro · Actif</p>
      </div>
    </div>
    ...
    <div className="space-y-1 mb-6">
      {menuItems.map((item) => (
        <Link
          key={item.path}
          to={item.path}
          className="glass-card rounded-xl p-3.5 flex items-center gap-3 hover:bg-secondary/50 transition-colors"
        >
    ...
    <button className="w-full glass-card rounded-xl p-3.5 flex items-center gap-3 text-destructive hover:bg-destructive/10 transition-colors">
